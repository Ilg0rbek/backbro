...
class UserService {
  ...
  public async findAllUser(): Promise<User[]> {
    const users: User[] = await this.users.find().populate({path: 'staff'}).populate({path: 'role', select: 'title_uz title_en title_ru'}).exec();
    return users;
  }
  ...
  public async updateUser(userId: string, userData: CreateUserDto): Promise<User> {
    ...
    if (userData.username) {
      const findUser: User = await this.users.findOne({ username: userData.username });
      if (findUser && findUser._id != userId) throw new HttpException(409, `You're username ${userData.username} already exists`);
    }
